...
based API to transform types and and various ways to create
...
developed in a literate programming style and is in active, regular
development since late 2011, currently in its 4th iteration/rewrite
cycle.

*You're highly encouraged to consult the source code, which contains documentation, examples, diagrams and general usage pattern hints and is increasingly being cross-linked between namespaces.*
...
A growing number (currently 35+) of small examples are included in
...
Growing list of [[http://workshop.thi.ng/][thi.ng workshop]] repositories (These workshops are
running on a monthly basis & internationally):

- [[http://thi.ng/ws-ldn-1][WS-LDN-1]]
- [[http://thi.ng/ws-ldn-2][WS-LDN-2]]
- [[http://thi.ng/ws-bln-1][WS-BLN-1]]
- WS-LDN-5 (forthcoming)
- WS-LDN-6 (forthcoming)
...
- OpenGL helpers for desktop app (wrapping [[http://jogamp.org/jogl][JOGL]])
...
- Parallel-transport frame sweep mesh generation from point sequences (skinning)
...
|                | Perspective camera              |
|                | FBO abstractions (OpenGL/WebGL) |
|                | Texture2D (OpenGL/WebGL)        |

*** API scope

Since the core library does only deal with pure "platonic" geometry,
it doesn't directly address any display or rendering functionality at
all. However a number of support modules are provided, incl. OpenGL
3/4, WebGL & SVG support. 2D/3D shape/mesh exporters are provided as
well and together with sister libraries like [[https://github.com/thi-ng/luxor][thi.ng/luxor]], it's also
possible to generate complete 3D scenes for high quality & high
resolution offline rendering using [[http://luxrender.net][Luxrender]].
...
| =thi.ng.geom.webgl.animator=     | =thi.ng.geom.gl.webgl.animator=  |
|----------------------------------+----------------------------------|
|                                  | =thi.ng.geom.attribs=            |
