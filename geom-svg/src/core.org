...
#+BEGIN_SRC clojure :noweb-ref prims
  ...
  (defn line-decorated
    ...
    (list
     (line p q attribs)
     (if start (start q p attribs))
     (if end   (end p q attribs))))
  ...
  (defn line-strip-decorated
    ...
    (list
     ...
     (if start (start (points 1) (points 0) attribs))
     (if seg   (map (fn [[p q]] (seg p q attribs)) (d/successive-nth 2 (pop points))))
     (if end   (end (points (- (count points) 2)) (peek points) attribs))))
...
#+BEGIN_SRC clojure :noweb-ref adapter
  (extend-protocol svg/PSVGConvert
    ...
    (as-svg
      ...
      (if (or start segment end)
        (svg/line-strip-decorated
         points start segment end
         (dissoc opts :start :segment :end))
