...
#+BEGIN_SRC clojure :noweb-ref prims
  ...
  (defn line-decorated
    ...
    (cond-> (list (line p q attribs))
            start (conj (start q p attribs))
            end   (conj (start p q attribs))))
  ...
  (defn line-strip-decorated
    [points start end & [attribs]]
    (cond-> (list (line-strip points attribs))
            start (conj (start (points 1) (points 0) attribs))
            end   (conj (end (points (- (count points) 2)) (peek points) attribs))))
...
#+BEGIN_SRC clojure :noweb-ref adapter
  (extend-protocol svg/PSVGConvert
    ...
    (as-svg
      [{p :points} {:keys [start end] :as opts}]
      (if (or start end)
        (svg/line-decorated (p 0) (p 1) start end (dissoc opts :start :end))
    ...
    (as-svg
      [{:keys [points]} {:keys [start segment end] :as opts}]
      (cond
       (and (or start end) (not segment))
       (svg/line-strip-decorated points start end (dissoc opts :start :end))

       segment
       (let [opts (dissoc opts :start :end :segment)]
         (cond-> (list
                  (svg/line-strip points opts)
                  (map (fn [[p q]] (segment p q opts)) (d/successive-nth 2 (pop points))))
                 start (conj (start (points 1) (points 0) opts))))
...
#+BEGIN_SRC clojure :tangle ../babel/src/cljx/thi/ng/geom/svg/core.cljx :noweb yes :mkdirp yes :padline no
  (ns thi.ng.geom.svg.core
    (:require
     ...
     [thi.ng.geom.core.vector :refer [vec2 vec3 V3Z]]
