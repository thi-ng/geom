...
[[http://media.thi.ng/geom/viz/lineplot-polar.svg]]
...
#+BEGIN_SRC clojure :noweb-ref plot-2d
  ...
  (defn svg-x-axis-cartesian
    [{:keys [scale major minor major-size minor-size label-dist pos format] [r1 r2] :range
      :as spec}]
    (svg-axis*
...
#+BEGIN_SRC clojure :noweb-ref plot-2d
  (defn svg-x-axis-polar
    [{{:keys [scale major minor major-size minor-size label-dist pos format]
       [r1 r2] :range
