...
#+BEGIN_SRC clojure :tangle ../babel/examples/lineplot-polar.clj :mkdirp yes :padline no
  ...
  (->> {:x-axis (viz/linear-axis
                 {:domain [(- PI) PI] :range [(* 1.1 PI) (* 1.9 PI)]
                  ...
                  :pos 280})
        :y-axis (viz/linear-axis
                 {:domain [-1 1] :range [60 280] :major 0.5 :minor 0.25
                  :major-size 0.05
                  :minor-size 0.025
                  :pos (* 1.1 PI)})
...
| [[http://media.thi.ng/geom/viz/lens-focus-2.gif]] | [[http://media.thi.ng/geom/viz/lens-strength-3.gif]] |
...
#+BEGIN_SRC clojure :noweb-ref axis
  ...
  (defn linear-axis
    [{:keys [domain range major minor major-size minor-size visible]
      :or {visible true, major-size 10, minor-size 5} :as spec}]
...
#+BEGIN_SRC clojure :noweb-ref axis
  ...
  (defn log-axis
    [{:keys [base domain range major-size minor-size visible]
      :or {visible true, major-size 10, minor-size 5} :as spec}]
    (assoc spec
...
#+BEGIN_SRC clojure :noweb-ref axis
  (defn lens-axis
    [{:keys [domain range focus strength major minor major-size minor-size visible]
      :or {visible true, strength 0.5, major-size 10, minor-size 5} :as spec}]
    (let [major' (if major (lin-tick-marks domain major))
          minor' (if minor (lin-tick-marks domain minor))
          minor' (if (and major' minor')
                   (filter (complement (set major')) minor')
...
#+BEGIN_SRC clojure :noweb-ref plot-2d
  (defn svg-axis*
    [{:keys [major minor]} axis align tick1-fn tick2-fn]
    (svg/group
     {:stroke "black"}
     (svg/group
      {:fill "black"
  ...
  (defn svg-x-axis-cartesian
    [{:keys [scale major minor major-size minor-size pos format] [r1 r2] :range
      :or {format (value-formatter 2)} :as spec}]
    (svg-axis*
     spec (svg/line [r1 pos] [r2 pos]) "start"
     #(let [x (scale %)]
        [(svg/line [x pos] [x (+ pos major-size)]) (svg/text [x (+ pos 20)] (format %) {:stroke "none"})])
     #(let [x (scale %)]
        (svg/line [x pos] [x (+ pos minor-size)]))))

  (defn svg-y-axis-cartesian
    [{:keys [scale major minor major-size minor-size pos format] [r1 r2] :range
      :or {format (value-formatter 2)} :as spec}]
    (svg-axis*
     spec (svg/line [pos r1] [pos r2]) "end"
     #(let [y (scale %)]
        [(svg/line [pos y] [(- pos major-size) y]) (svg/text [(- pos 15) y] (format %) {:stroke "none"})])
     #(let [y (scale %)]
        (svg/line [pos y] [(- pos minor-size) y]))))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref plot-2d
  (defn svg-x-axis-polar
    [{{:keys [scale major minor major-size minor-size pos format]
       [r1 r2] :range
       :or {format (value-formatter 2)}} :x-axis
       project :project o :origin :as spec}]
    (svg-axis*
     (:x-axis spec) (svg/arc o pos r1 r2 true {:fill "none"}) "start"
     #(let [x (scale %)]
        [(svg/line (project [x pos]) (project [x (- pos major-size)]))
         (svg/text (project [x (- pos major-size)]) (format %) {:stroke "none"})])
     #(let [x (scale %)]
        (svg/line (project [x pos]) (project [x (- pos minor-size)])))))

  (defn svg-y-axis-polar
    [{{:keys [scale major minor major-size minor-size pos format] [r1 r2] :range
       :or {format (value-formatter 2)}} :y-axis
       project :project :as spec}]
    (svg-axis*
     (:y-axis spec) (svg/line (project [pos r1]) (project [pos r2])) "end"
     #(let [y (scale %)]
        [(svg/line (project [pos y]) (project [(- pos major-size) y]))
         (svg/text (project [(- pos major-size) y]) (format %) {:stroke "none"})])
     #(let [y (scale %)]
