...
#+BEGIN_SRC clojure :tangle babel/src-cljx/thi/ng/geom/mesh.cljx
  ...
  (defn subdivide*
    [mesh {:keys [add scale mix]} {:keys [vertex displace splits] flt :filter}]
    (let [vertex (or vertex #(t/centroid* add scale %))]
      (reduce
       (fn [mesh f] (subdivide-face* mesh mix f (vertex f) displace splits))
       mesh (if flt (filter flt (:faces mesh)) (:faces mesh)))))
#+END_SRC
** Constructors
#+BEGIN_SRC clojure :tangle babel/src-cljx/thi/ng/geom/mesh.cljx
  (defn mesh2
...
#+BEGIN_SRC clojure :tangle babel/src-cljx/thi/ng/geom/mesh.cljx
  g/ISliceable
  (slice-with
    ([this e] (g/slice-with this e (:faces this)))
    ([this e faces]
       (let [verts (persistent!
         ...
         (reduce
          (fn [[this nf :as state] f]
            (if-let [tfaces (t/slice-with f e classifier)]
              [(reduce g/add-face
                       (reduce g/add-face (g/remove-face this f) (get tfaces -1))
                       (get tfaces 1))
