...
#+BEGIN_SRC clojure :tangle babel/src-cljx/thi/ng/geom/polygon.cljx
  ...
  (area
   [{points :points}]
   (let [p (first points)
         [area] (reduce (fn [[a p] v] [(+ a (g/cross p v)) v])
  ...
  (center
   ...
   ([this o]
      (thi.ng.geom.types.Polygon. (vec (g/translate (g/sub o (g/centroid this)) (:points this))) [])))
  (centroid
   ...
   (let [p (first points)
         [c] (reduce
              (fn [[c p] v]
                (let [x (g/cross p v)]
                  [(g/add c (g/scale-n (g/add p v) x)) v]))
              [[0.0 0.0] p] (conj (subvec points 1 (count points)) p))]
     (g/scale-n c (/ 1.0 (* 6 (g/area this))))))
  (width
