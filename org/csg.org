       ...
       (let [dir (g/sub e s)
             ...
             f (fn [stack i blend r]
                 (let [theta (* m/TWO_PI i)
                       out (g/madd ax (Math/cos theta) (g/scale-n ay (Math/sin theta)))
                       norm (g/madd out (- 1.0 (m/abs blend)) (g/scale-n az blend))
                       pos (g/add s (g/scale-n dir stack) (g/scale-n out r))]
                   (csg-vertex pos norm)))
             res (double res)]
         (mapcat
          (fn [i]
