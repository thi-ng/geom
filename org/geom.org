...
#+BEGIN_SRC clojure :tangle babel/src-cljx/thi/ng/geom/core.cljx
       ...
       (let [c (count coll)]
         (cond
          (> c 1) (let [p (apply min2 coll)
                        ...
                        [w h] (sub2 q p)]
                    (types/->Rect p w h))
          (= c 1) (types/->Rect (first coll) 0 0)
          :default nil)))
    ([p w h] (types/->Rect p w h)))
       ...
       (let [cnt (count coll)]
         (cond
          (> cnt 1) (let [p (apply min3 coll)
                          q (apply max3 coll)]
                      (types/->AABB p q))
          (= cnt 1) (types/->AABB (first coll) [0 0 0])
          :default nil)))
    ([p q] (types/->AABB p (sub3 q p))))
  ...
  (defn bounding-circle*
    [c r-or-points]
    (types/->Circle c (if (coll? r-or-points)
                        (max-dist sub2 mag2-squared c r-or-points)
                        r-or-points)))

  (defn bounding-sphere*
    [c r-or-points]
    (types/->Sphere c (if (coll? r-or-points)
...
(defn circle
  ([] (Circle. [0.0 0.0] 1.0))
  ([r] (Circle. [0.0 0.0] r))
  ([p r] (Circle. (g/vec2 p) r))
  ([x y r] (Circle. (g/vec2 x y) r)))
#+END_SRC
...
#+BEGIN_SRC clojure :tangle babel/src-cljx/thi/ng/geom/line.cljx
  (defn line2
    ([p q] (Line2. (g/vec2 p) (g/vec2 q)))
    ([px py qx qy] (Line2. (g/vec2 px py) (g/vec2 qx qy))))

  (defn line3
    ([p q] (Line3. (g/vec3 p) (g/vec3 q)))
    ([px py qx qy] (Line3. (g/vec3 px py) (g/vec3 qx qy)))
    ([px py pz qx qy qz] (Line3. (g/vec3 px py pz) (g/vec3 qx qy qz))))
#+END_SRC
...
#+BEGIN_SRC clojure :tangle babel/src-cljx/thi/ng/geom/line.cljx
  ...
  (center
   ([{p :p q :q}]
      (let [c (g/mid2 p q)] (types/->Line2 (g/sub2 p c) (g/sub2 q c))))
   ([{p :p q :q} o]
      (let [c (g/sub2 o (g/mid2 p q))] (types/->Line2 (g/add2 p c) (g/add2 q c)))))
  (centroid
