...
****** TODO merge all into ITransformable??
****** IRotatable
...
****** ITessellateable
#+BEGIN_SRC clojure :tangle babel/src-cljx/thi/ng/geom/core.cljx
  (defprotocol ITessellateable
    (tessellate [this] [this opts]))
#+END_SRC
****** IExtrudeable
#+BEGIN_SRC clojure :tangle babel/src-cljx/thi/ng/geom/core.cljx
  (defprotocol IExtrudeable
    (extrude [this opts]))
#+END_SRC
...
#+BEGIN_SRC clojure :tangle babel/src-cljx/thi/ng/geom/core.cljx
  ...
  (defn- vecop3
    ...
    (fn
      ...
      ([v x y]
         (if (number? x)
           [(f (nth v 0) x) (f (nth v 1) y) (nth v 2)]
           (let [x (vec3 x) y (vec3 y)]
             [(f (f (nth v 0) (x 0)) (y 0))
              (f (f (nth v 1) (x 1)) (y 1))
              (f (f (nth v 2) (x 2)) (y 2))])))
      ([v x y z]
  ...
  (defn- vecop4
    ...
    (fn
      ...
      ([v x y]
         (if (number? x)
           [(f (nth v 0) x) (f (nth v 1) y) (nth v 2) (nth v 3)]
           (let [x (vec4 x) y (vec4 y)]
             [(f (f (nth v 0) (x 0)) (y 0))
              (f (f (nth v 1) (x 1)) (y 1))
              (f (f (nth v 2) (x 2)) (y 2))
              (f (f (nth v 3) (x 3)) (y 3))])))
      ([v x y z]
         (if (number? x)
           [(f (nth v 0) x) (f (nth v 1) y) (f (nth v 2) z) (nth v 3)]
           (let [x (vec4 x) y (vec4 y) z (vec4 z)]
             [(f (f (f (f (nth v 0) (x 0)) (y 0)) (z 0)))
              (f (f (f (f (nth v 1) (x 1)) (y 1)) (z 1)))
              (f (f (f (f (nth v 2) (x 2)) (y 2)) (z 2)))
              (f (f (f (f (nth v 3) (x 3)) (y 3)) (z 3)))])))
      ([v x y z w]
         (if (number? x)
...
#+BEGIN_SRC clojure :tangle babel/src-cljx/thi/ng/geom/core.cljx
  ...
  (defn normal3
    ([a b c] (normalize3 (cross3 (sub3 b a) (sub3 c a))))
    ([[a b c]] (normalize3 (cross3 (sub3 b a) (sub3 c a)))))
