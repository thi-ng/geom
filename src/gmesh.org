...
#+BEGIN_SRC clojure :tangle ../babel/src/cljx/thi/ng/geom/gmesh.cljx
  ...
  (defn lathe-mesh
    "Takes a seq of vec3's and treats them as lathe to form quad mesh.
    Lathe is revolved around an axis using given rotation fn and radial
    resolution. The rotation fn must accept two args: a point & rotation
    angle. Optionally, each resulting quad can also be slightly scaled
    (i.e. to create gaps), so resulting mesh might not be continous."
    [points rot-fn res & [scale]]
    (let [strips (mapv
                  (fn [i]
                    (let [theta (* i m/TWO_PI)]
          ...
          make-face (if scale
                      (fn [[a1 a2] [b1 b2]] (g/scale-size* scale [b1 b2 a2 a1]))
                      (fn [[a1 a2] [b1 b2]]
                        (cond
                         (= a1 b1) [b1 b2 a2]
                         (= a2 b2) [b1 a2 a1]
                         :default [b1 b2 a2 a1])))]
      (->> (d/successive-nth 2 (conj strips (first strips)))
           (mapcat
            (fn [[sa sb]]
              (map make-face
