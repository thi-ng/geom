...
#+BEGIN_SRC clojure :tangle ../babel/src/cljx/thi/ng/geom/mesh/subdivision.cljx
  (defn bf-face-point
    [f a b] (some #(if (and (not= % a) (not= % b)) %) f))
  
  (defn bf-butterfly-point
    [edges a b f]
    (bf-face-point (first (disj (edges #{a b}) f)) a b))
           ...
           (r/map
            (fn [[e e-faces]]
              (let [[a b] (seq e)
                    [efa efb] (seq e-faces)
                    e1 (g/mix a b)
                    d3 (bf-face-point efa a b)
                    d4 (bf-face-point efb a b)]
                (if (and d3 d4)
                  (let [d6 (bf-butterfly-point edges a d3 efa)
                        d5 (bf-butterfly-point edges b d3 efa)
                        d8 (bf-butterfly-point edges a d4 efb)
                        d7 (bf-butterfly-point edges b d4 efb)]
                    (if (and d5 d6 d7 d8)
                      [e (g/+ e1
                              (g/addm d3 d4 omega)
                              (g/addm (g/+ d5 d6) (g/+ d7 d8) om2))]
                      [e e1]))
                  [e e1]))))
           (into {}))))
