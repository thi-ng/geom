...
[[https://devart.withgoogle.com/#/project/16501507][Google DevArt commission]] and a generative design workshop at [[http://resonate.io][Resonate]],
beginning of April.
...
results in a 3D model file in Stanford PLY format and can then be
           ...
           (-> (c/circle x y 0.5)                    ;; create circle at X,Y radius=0.5
               ...
               (g/extrude-shell                      ;; convert into walled, cylindrical mesh
                {:depth (m/random 0.25 2)
                 :wall 0.1})))
         (reduce g/into-mesh)                        ;; combine into single mesh
