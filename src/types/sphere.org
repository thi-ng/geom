    ...
    (let [slices  (or slices res)
      ...
      (loop [acc (transient []), fid 0, i 0, j 0]
        (if (< i slices)
          (let [ii (inc i)
                verts [[i j]]
                verts (if (pos? j) (conj verts [ii j]) verts)
                verts (if (< j (dec stacks)) (conj verts [ii (inc j)]) verts)
                verts (conj verts [i (inc j)])
                fverts (mapv
                        (fn [[u v]]
                          (vec3
                        ...
                        verts)
                face  (attr/generate-face-attribs
                       fverts fid attribs
                       {:uv (mapv #(vec2 (* iu (first %)) (* iv (nth % 1))) verts)})]
            (recur (conj! acc face) (inc fid) ii j))
          (if (< j stacks')
