  ...
  (defn altitude
    ([[a b c] id]
       (case id
         ...
         :c (altitude a b c)))
    ([a b c]
       [(g/mix a b (l/closest-point-coeff* c a b)) c]))
  ...
  (defn norm-altitude
    ([points id]
       (let [[a b] (altitude points id)]
         (g/normalize (g/- b a))))
    ([a b c]
       (g/normalize (g/- c (g/mix a b (l/closest-point-coeff* c a b))))))
...
**** PMeshConvert                                                       :fix:
#+BEGIN_SRC clojure
  ;; :tangle ../babel/src/cljx/thi/ng/geom/triangle.cljx
  g/PMeshConvert
  (as-mesh [_] (g/add-face (gm/gmesh) (mapv vec3 (:points _))))
#+END_SRC
