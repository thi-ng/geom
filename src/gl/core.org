...
#+BEGIN_SRC clojure :noweb-ref helpers
  ...
  (defn face-normals-buffer
    [faces]
    (let [buf (#?(:clj native/float-buffer :cljs ta/float32) (* (count faces) 3 3))]
  ...
  (defn face-normals
    ...
    (let [fnorms (g/face-normals m true)
          faces  (g/faces m)
          buf    (#?(:clj native/float-buffer :cljs ta/float32) (* (count faces) 9))]
      (loop [idx 0, faces faces]
        (if faces
          #?(:clj
      ...
      (cond->
       ...
       (assoc-in
        ...
        {:data (fill-buffer-vec3
                (#?(:clj native/float-buffer :cljs ta/float32) (* num-vertices 3))
       ...
       (assoc-in
        ...
        {:data (fill-vertex-buffer
                (#?(:clj native/float-buffer :cljs ta/float32) (* 2 (count uv))) uv 2)
         :size 2})
       ...
       (assoc-in
        [:attribs :color]
        {:data (fill-vertex-buffer
                (#?(:clj native/float-buffer :cljs ta/float32) (* c-stride (count colors)))
                colors c-stride)
         :size c-stride})
       ...
       (assoc-in
        [:attribs :color]
        {:data (fill-buffer
                (#?(:clj native/float-buffer :cljs ta/float32) (* num-vertices c-stride))
...
#+BEGIN_SRC clojure :noweb-ref ctx
  ...
  #?(:clj
     (defn get-viewport-rect
       [^GL3 gl]
       (let [b (.glGetParameter gl glc/viewport)]
     ...
     (defn make-vertex-array
       ...
       (let [^IntBuffer id (native/int-buffer 1)]
         ...
         (doseq [[attr {:keys [id loc offset size stride]
           ...
           (let [loc (int (or loc (get-in spec [:shader :attribs attr])))]
             (doto gl
...
#+BEGIN_SRC clojure :noweb-ref draw
  (defn begin-shader
    ...
    (#?(:clj .glUseProgram :cljs .useProgram) gl (int (get shader :program)))
    ...
    (when indices
      (#?(:clj .glBindBuffer :cljs .bindBuffer) gl glc/element-array-buffer (get indices :buffer))))
