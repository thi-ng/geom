...
#+BEGIN_SRC clojure :noweb-ref ctors
  (defn gl-mesh
    ...
    ([numf attribs]
    (GLMesh.
      (#?(:clj native/float-buffer :cljs ta/float32) (* numf 9))
      (if (get attribs :fnorm)
        (#?(:clj native/float-buffer :cljs ta/float32) (* numf 9)))
      (if (get attribs :vnorm)
        (#?(:clj native/float-buffer :cljs ta/float32) (* numf 9)))
      (if (get attribs :uv)
        (#?(:clj native/float-buffer :cljs ta/float32) (* numf 6)))
      (if (get attribs :col)
        (#?(:clj native/float-buffer :cljs ta/float32) (* numf 12)))
      0
      attribs)))
  ...
  (defn indexed-gl-mesh
    ...
    ([numf attribs]
     (IndexedGLMesh.
      ...
      (if (get attribs :col)
        (#?(:clj native/float-buffer :cljs ta/float32) (* numf 12)))
      (#?(:clj native/short-buffer :cljs ta/uint16) (* numf 3))
      attribs
      {} 0 0)))
#+END_SRC
