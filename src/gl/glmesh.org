...
* Contents                                                         :toc_4_gh:
 - [[#namespace-thinggeomglglmesh][Namespace: thi.ng.geom.gl.glmesh]]
     ...
     - [[#indexedglmesh][IndexedGLMesh]]
         ...
         - [[#end-of-implementation][End of implementation]]
     - [[#helpers][Helpers]]
         - [[#adding-mesh-faces][Adding mesh faces]]
...
#+BEGIN_SRC clojure :noweb-ref ctors
  (defn gl-mesh
    ...
    ([numf attribs]
    (GLMesh.
      (#?(:clj gl/float-buffer :cljs ta/float32) (* numf 9))
      (if (get attribs :fnorm)
        (#?(:clj gl/float-buffer :cljs ta/float32) (* numf 9)))
      (if (get attribs :vnorm)
        (#?(:clj gl/float-buffer :cljs ta/float32) (* numf 9)))
      (if (get attribs :uv)
        (#?(:clj gl/float-buffer :cljs ta/float32) (* numf 6)))
      (if (get attribs :col)
        (#?(:clj gl/float-buffer :cljs ta/float32) (* numf 12)))
      0
      attribs)))
  ...
  (defn indexed-gl-mesh
    ...
    ([numf attribs]
     (IndexedGLMesh.
      ...
      (if (get attribs :col)
        (#?(:clj gl/float-buffer :cljs ta/float32) (* numf 12)))
      (#?(:clj gl/short-buffer :cljs ta/uint16) (* numf 3))
      attribs
      {} 0 0)))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref helpers
  (defn- add-face*
    ...
    (if (> (count verts) 3)
      ...
      (let [vertices (.-vertices m)
        ...
        #?@(:clj
            [(->> idv
                  (gl/into-buffer-vec3 (first verts) vertices 3)
                  (gl/into-buffer-vec3 (nth verts 1) vertices 3)
                  (gl/into-buffer-vec3 (nth verts 2) vertices 3))]
            :cljs
        ...
        (when fnormals
          #?(:clj
             (let [n  (or (get attribs :fnorm) (gu/ortho-normal verts))]
               (->> idv
                    (gl/into-buffer-vec3 n fnormals 3)
                    (gl/into-buffer-vec3 n fnormals 3)
                    (gl/into-buffer-vec3 n fnormals 3)))
             :cljs
             (let [n  (or (get attribs :fnorm) (gu/ortho-normal verts))
        ...
        (when-let [vn (if vnormals (get attribs :vnorm))]
          #?@(:clj
              [(->> idv
                    (gl/into-buffer-vec3 (first vn) vnormals 3)
                    (gl/into-buffer-vec3 (nth vn 1) vnormals 3)
                    (gl/into-buffer-vec3 (nth vn 2) vnormals 3))]
              :cljs
        ...
        (when-let [uv (if uvs (get attribs :uv))]
          #?@(:clj
              [(->> iduv
                    (gl/into-buffer-vec2 (first uv) uvs 2)
                    (gl/into-buffer-vec2 (nth uv 1) uvs 2)
                    (gl/into-buffer-vec2 (nth uv 2) uvs 2))]
              :cljs
        ...
        (when-let [col (if cols (get attribs :col))]
          (->> (streams/into-buffer (first col) cols 4 idcol) ;; FIXME
  ...
  (defn- index-vertex*
    ...
    (or (get (g/get-prop m :index) va)
        (let [index #?(:clj (g/get-prop m :index) :cljs (.-index m))
              ...
              idv   (* id 3)
              [v fn vn col uv] va]
          #?(:clj  (gl/into-buffer-vec3 v vertices 3 idv)
             :cljs (.set vertices (.-buf ^thi.ng.geom.vector.Vec3 v) idv))
          (if (if fnormals fn)
            #?(:clj  (gl/into-buffer-vec3 fn fnormals 3 idv)
               :cljs (.set fnormals (.-buf ^thi.ng.geom.vector.Vec3 fn) idv))
            (if (if vnormals vn)
              #?(:clj  (gl/into-buffer-vec3 vn vnormals 3 idv)
                 :cljs (.set vnormals (.-buf ^thi.ng.geom.vector.Vec3 vn) idv))))
          (when (if cols col)
            (streams/into-buffer col cols 4 (* id 4))) ;; FIXME
          (when (if uvs uv)
            #?(:clj  (gl/into-buffer-vec2 uv uvs 2 (* id 2))
         ...
         (when (< i num)
           #?@(:clj
               [(aset tb 0 (.get buf i))
                ...
                (gl/into-buffer-vec3 (f tv) buf 3 i)]
               :cljs
  ...
  (defn- into-glmesh*
    ...
    (let [{sverts :vertices sfn :fnormals svn :vnormals scol :cols suv :uvs sid :id} src
          ...
          sidv   (* sid 9)
          didv   (* did 9)]
      #?(:clj  (gl/copy-float-buffer dverts sverts didv 0 sidv)
         :cljs (.set dverts (.slice sverts 0 sidv) didv))
      (when (if dfn sfn)
        #?(:clj  (gl/copy-float-buffer dfn sfn didv 0 sidv)
           :cljs (.set dfn (.slice sfn 0 sidv) didv)))
      (when (if dvn svn)
        #?(:clj  (gl/copy-float-buffer dvn svn didv 0 sidv)
           :cljs (.set dvn (.slice svn 0 sidv) didv)))
      (when (if dcol scol)
        #?(:clj  (gl/copy-float-buffer dcol scol (* did 12) 0 (* sid 12))
           :cljs (.set dcol (.slice scol 0 (* sid 12)) (* did 12))))
      (when (if duv suv)
        #?(:clj  (gl/copy-float-buffer duv suv (* did 6) 0 (* sid 6))
        ...
        (when (< i sidv)
          (if-let [nid (first (get rindex #?(:clj (.get ^ShortBuffer sidx i) :cljs (aget sidx i))))]
            (let [sid    #?(:clj (.get ^ShortBuffer sidx i) :cljs (aget sidx i))
              ...
              #?(:clj  (gl/copy-float-buffer-vec3 dverts sverts didv sidv)
                 :cljs (.set dverts (.slice sverts sidv (+ sidv 3)) didv))
              (when fn?
                #?(:clj  (gl/copy-float-buffer-vec3 dfn sfn didv sidv)
                   :cljs (.set dfn (.slice sfn sidv (+ sidv 3)) didv)))
              (when vn?
                #?(:clj  (gl/copy-float-buffer-vec3 dvn svn didv sidv)
                   :cljs (.set dvn (.slice svn sidv (+ sidv 3)) didv)))
              (when col?
                #?(:clj  (gl/copy-float-buffer-vec4 dcol scol (* nid 4) sidcol)
                   :cljs (.set dcol (.slice scol sidcol (+ sidcol 4)) (* nid 4))))
              (when uv?
                #?(:clj  (gl/copy-float-buffer-vec2 duv suv (* nid 2) siduv)
...
#+BEGIN_SRC clojure :tangle ../../babel/src/thi/ng/geom/gl/glmesh.cljc :noweb yes :mkdirp yes :padline no
