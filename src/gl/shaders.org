...
#+BEGIN_SRC clojure :noweb-ref uniforms
  (def uniform-types
    #?(:clj  {:float       [#(.glUniform1f ^GL3 % %2 %3) float 1]
              ...
              :vec2        [#(.glUniform2fv ^GL3 % %2 1 %3) native/float-buffer 2]
              :vec3        [#(.glUniform3fv ^GL3 % %2 1 %3) native/float-buffer 3]
              :vec4        [#(.glUniform4fv ^GL3 % %2 1 %3) native/float-buffer 4]
              :ivec2       [#(.glUniform2iv ^GL3 % %2 1 %3) native/int-buffer 2]
              :ivec3       [#(.glUniform3iv ^GL3 % %2 1 %3) native/int-buffer 3]
              :ivec4       [#(.glUniform4iv ^GL3 % %2 1 %3) native/int-buffer 4]
              :bool        [#(.glUniform1i ^GL3 % %2 %3) int 1] ;; TODO
              :bvec2       [#(.glUniform2iv ^GL3 % %2 1 %3) native/int-buffer 2] ;; TODO bool -> int
              :bvec3       [#(.glUniform3iv ^GL3 % %2 1 %3) native/int-buffer 3]
              :bvec4       [#(.glUniform4iv ^GL3 % %2 1 %3) native/int-buffer 4]
              :mat2        [#(.glUniformMatrix2fv ^GL3 % %2 1 %3 %4) native/float-buffer 4]
              :mat3        [#(.glUniformMatrix3fv ^GL3 % %2 1 %3 %4) native/float-buffer 9]
              :mat4        [#(.glUniformMatrix4fv ^GL3 % %2 1 %3 %4) native/float-buffer 16]
              :sampler2D   [#(.glUniform1i ^GL3 % %2 %3) int 1]
              :samplerCube [#(.glUniform1i ^GL3 % %2 %3) int 1]}
       :cljs {:float       ["1fv" ta/float32]
              :int         ["1iv" ta/int32]
...
#+BEGIN_SRC clojure :noweb-ref uniforms
    ...
    (reduce-kv
     (fn [umap id type]
       (let [loc                   #?(:clj  (.glGetUniformLocation gl prog (name id))
             ...
             #?@(:cljs [setter (aget gl (str "uniform" u-type))])]
         (assoc
          umap id
          {:type    type
           :default default
           :setter  (cond
                      ...
                      (fn [x]
                        #?(:clj
                           (setter gl loc (if opt glc/gl-true glc/gl-false)
  ...
  (defn set-uniform
    ...
    (if-let [u-spec (get-in shader [:uniforms id])]
      ...
      (#?(:clj println :cljs warn) "Unknown shader uniform: " id)))
...
#+BEGIN_SRC clojure :noweb-ref attribs
  ...
  (defn set-attribute
    ...
    (let [{:keys [buffer stride size type normalized? offset]} attrib-spec]
      (if-let [loc (-> shader (get :attribs) (get id))]
        (doto gl
          (#?(:clj .glBindBuffer :cljs .bindBuffer) glc/array-buffer buffer)
  ...
  (defn disable-attribute
    ...
    (if-let [loc (-> shader (get :attribs) (get id))]
      (do (#?(:cljs .glDisableVertexAttribArray :cljs .disableVertexAttribArray) gl loc) gl)
      (#?(:clj println :cljs warn) (str "Unknown shader attribute: " id))))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref ctors
  ...
  (defn prepare-shader-sources
    ...
    (let [gl3?     (and version (>= version 300))
          ...
          a-src    (compile-glsl-vars (if gl3? "in" "attribute") attribs)
  ...
  #?(:clj
     ...
     (defn make-shader-from-spec
       ...
       (let [{:keys [vs fs uniforms attribs]} spec
