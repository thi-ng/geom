...
#+BEGIN_SRC clojure :noweb-ref attribs
  ...
  (defn set-attribute
    ...
    (let [{:keys [buffer stride size type normalized? offset loc]} attrib-spec]
      ;;(prn :loc id (or loc (-> shader (get :attribs) (get id))) :size size stride)
      (if-let [loc (int (or loc (-> shader (get :attribs) (get id))))]
        (doto gl
     ...
     (defn make-shader-from-spec
       ...
       (let [{:keys [vs fs uniforms attribs]} spec
         ...
         (doto gl
           ...
           (bind-attrib-locations prog-id attribs)
           (.linkProgram prog))
