...
#+BEGIN_SRC clojure :noweb-ref uniforms
  (def uniform-types
    #?(:clj  {:float       [#(.glUniform1f ^GL3 % %2 %3) float 1]
              ...
              :bvec2       [#(.glUniform2iv ^GL3 % %2 1 %3) bool->ivec 2]
              :bvec3       [#(.glUniform3iv ^GL3 % %2 1 %3) bool->ivec 3]
              :bvec4       [#(.glUniform4iv ^GL3 % %2 1 %3) bool->ivec 4]
    ...
    (reduce-kv
     (fn [umap id type]
       (let [loc                   #?(:clj  (.glGetUniformLocation gl prog (name id))
         ...
         (#?(:clj prn :cljs debug) :uniform-loc id loc)
         (assoc
          umap id
          {:type    type
           :default default
