...
#+BEGIN_SRC clojure :noweb-ref fbo
  (defrecord FBO
    ...
    gl/IBind
    (bind [_]
      #?(:clj  (.glBindFramebuffer gl glc/framebuffer (.get ^IntBuffer id 0))
...
#+BEGIN_SRC clojure :noweb-ref render-buffer
  (defrecord RenderBuffer
      #?(:clj  [^GL3 gl id format width height]
         :cljs [^WebGLRenderingContext gl id format width height])
    gl/IBind
    (bind [_]
      #?(:clj  (.glBindRenderbuffer gl glc/renderbuffer (.get ^IntBuffer id 0))
