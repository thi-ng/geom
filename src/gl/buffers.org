...
* Contents                                                         :toc_4_gh:
 - [[#thinggeomglbuffers][thi.ng.geom.gl.buffers]]
     ...
     - [[#textures][Textures]]
         - [[#todo-currently-webgl-only][TODO Currently WebGL only]]
...
#+BEGIN_SRC clojure :noweb-ref tex
  (defrecord Texture2D
    ...
    gl/IRelease
    (release [_]
      #?(:cljs (.glDeleteTextures gl 1 id) :cljs (.deleteTexture gl id))
      _)
    gl/IConfigure
    (configure
      ...
      #?(:clj
         ...
         (cond
           ...
           (and pixels width height) (.texImage2D gl target 0 format width height 0 format type pixels)))
      _))
  ...
  #?(:clj
     (defn make-texture
       ...
       (let [id   (native/int-buffer 1)
             ...
             tex  (Texture2D. gl id (opts :target glc/texture-2d))
