...
#+BEGIN_SRC clojure :tangle babel/src-cljx/thi/ng/geom/polygon.cljx
  ...
  (extrude
      ...
      (let [points (if (pos? (g/area this)) (reverse points) points)
            tpoints (if (= 1.0 scale) points (:points (g/scale-size this scale)))
            off (or offset (vec3 0 0 depth))
            points3 (vec (map #(vec3 %) points))
            tpoints3 (vec (map #(g/add off %) tpoints))]
        (mesh/into-mesh
         (mesh/mesh3) nil
         (concat
          (when bottom?
          ...
          (mapcat (fn [[a1 b1] [a2 b2]] [[b1 a1 a2] [b2 b1 a2]])
