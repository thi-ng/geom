...
#+BEGIN_SRC clojure :noweb-ref helpers
  (deftype CSGVertex [^thi.ng.geom.core.vector.Vec3 p ^thi.ng.geom.core.vector.Vec3 n]
    g/PInterpolate
    (mix [_ v t]
      (CSGVertex. (g/mix p (.-p ^CSGVertex v) t) (g/mix n (.-n ^CSGVertex v) t)))
    g/PFlip
    (flip [_]
      (CSGVertex. p (g/- n))))
  ...
  (deftype CSGPolygon [plane vertices shared ^:unsynchronized-mutable bsphere]
    ...
    (bounding-sphere
      [_] (if bsphere
            bsphere
            (do
              (set! bsphere (tu/bounding-sphere (mapv #(.-p ^CSGVertex %) vertices)))
              bsphere)))
    g/PFlip
    (flip [_]
      (CSGPolygon. (g/flip plane) (mapv g/flip (rseq vertices)) shared bsphere)))
  ...
  (defn compute-split-types
    ...
    (let [ieps (- *eps*)]
      (loop [ptype 0, types [], vertices vertices]
        (if vertices
          (let [t (- (g/dot n (.-p ^CSGVertex (first vertices))) w)
...
#+BEGIN_SRC clojure :noweb-ref api
  ...
  (defn csg-plane
    ...
    [[a b c]]
    (if (instance? CSGVertex a)
      (p/plane-from-points (.-p ^CSGVertex a) (.-p ^CSGVertex b) (.-p ^CSGVertex c))
      (p/plane-from-points a b c)))
  ...
  (defn csg-polygon
    ...
    ([vertices shared]
       (CSGPolygon. (csg-plane vertices) (vec vertices) shared nil)))
...
#+BEGIN_SRC clojure :noweb-ref ctors
  ...
  (defn csg->mesh
    [polygons]
    (->> polygons
         (map (fn [poly] (map #(.-p ^CSGVertex %) (.-vertices ^CSGPolygon poly))))
         (g/into (gm/gmesh))))
