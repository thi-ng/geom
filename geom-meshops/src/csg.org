...
#+BEGIN_SRC clojure :noweb-ref helpers
  ...
  (deftype CSGPolygon [plane vertices shared ^:unsynchronized-mutable bsphere]
    ...
    (bounding-sphere
      [_] (if bsphere
            bsphere
            (do (set! bsphere (tu/bounding-sphere (mapv :p vertices))) bsphere)))
    g/PFlip
    (flip [_]
  ...
  (defn split-poly
    ...
    [{:keys [n w]} ^CSGPolygon poly [cp-front cp-back front back]]
    (let [s (g/bounding-sphere poly)
          r (+ (:r s) *eps*)
          d (- (g/dot n (:p s)) w)]
      (cond
       (> d r) [cp-front cp-back (conj front poly) back]
       (< d (- r)) [cp-front cp-back front (conj back poly)]
       :default
