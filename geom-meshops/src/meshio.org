...
#+BEGIN_SRC clojure :noweb-ref obj
  ...
  (defn write-obj-indexed
    ...
    ([out mesh opts]
     (let [vertices    (-> mesh :vertices :items)
           vindex      (-> mesh :vertices :index)
           uvs         (-> mesh :attribs :uv :items)
           vnormals    (-> mesh :attribs :vnormals :items)
           faces       (-> mesh :faces)
           fmt-vertex  (obj-fmt3 "v")
           fmt-vnormal (obj-fmt3 "vn")
           fmt-uv      (obj-fmt2 "vt")]
       (when-let [mtl (:mtl-lib opts)]
         (streams/write-utf8-bytes
          out (str "mtllib " mtl "\nusemtl " (or (:mtl opts) "default") "\n\n")))
       (doseq [[x y z] vertices]
           ...
           (doseq [^thi.ng.geom.meshface.IndexedMeshFace f faces]
             (streams/write-utf8-bytes
              out (obj-fmt-face-v-uv-vn
                   (mapv inc (.-vertices f))
                   (mapv inc (:uv (.-attribs f)))
                   (mapv inc (:vnormals (.-attribs f))))))
           (doseq [^thi.ng.geom.meshface.IndexedMeshFace f faces]
             (streams/write-utf8-bytes
              out (obj-fmt-face-v-vn
                   (mapv inc (.-vertices f))
