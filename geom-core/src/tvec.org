...
#+BEGIN_SRC clojure :noweb-ref vec3
  g/PMinMax
  (min
   [_ v] (vm/v3-op1 min buf v _meta))
  (min
   [_ v v2] (vm/v3-op2 min min buf v v2 0.0 0.0 _meta))
  (max
   [_ v] (vm/v3-op1 max buf v _meta))
  (max
   [_ v v2] (vm/v3-op2 max max buf v v2 0.0 0.0 _meta))
#+END_SRC
*** PDeltaEquals
#+BEGIN_SRC clojure :noweb-ref vec3
  m/PDeltaEquals
