...
#+BEGIN_SRC clojure :noweb-ref vec3
  ...
  (mix
   ...
   (let [b' (js/Float32Array. 3)
         dv? (instance? Vec3 d)
         dx (if dv? (aget d 0) (nth d 0 0.0))
         dy (if dv? (aget d 1) (nth d 1 0.0))
         dz (if dv? (aget d 2) (nth d 2 0.0))]
     (vm/rewrite-v3-nv-nv
      buf b c 0.0 0.0
      (let [x1 (+ (* (- bx x) u) x)
            y1 (+ (* (- by y) u) y)
...
#+BEGIN_SRC clojure :tangle ../babel/src/cljx/thi/ng/geom/core/tvec.cljx :noweb yes :mkdirp yes :padline no
  ...
  (defn ^:export bench-mathop
    ...
    (let [tv (trace pre "v" (bench-op1 op v i))
      ...
      (apply str "+," (interpose "," [tv tn ts tvv tvn tvs tnv tnn tns tsv tsn tss]))))
