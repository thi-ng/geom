...
#+BEGIN_SRC clojure :noweb-ref vec3
  g/PDotProduct
  (dot
   [_ v] (vm/with-v3-or-vs-no-let buf v (+ (+ (* x vx) (* y vy)) (* z vz))))
#+END_SRC
*** PClear
#+BEGIN_SRC clojure :noweb-ref vec3'
  g/PClear
...
#+BEGIN_SRC clojure :noweb-ref vec3
  g/PMagnitude
  (mag
   [_] (vm/with-v3 buf (Math/sqrt (mm/madd x x y y z z))))
  (mag-squared
   [_] (vm/with-v3 buf (mm/madd x x y y z z)))
#+END_SRC
*** PNormalize
#+BEGIN_SRC clojure :noweb-ref vec3'
  g/PNormalize
    ...
    (let [a (gensym)
      ...
      `(let [~a ~src]
         (if (instance? ~'Vec3 ~v)
           ...
           ~@(postwalk-replace
              {'x `(aget ~a 0) 'y `(aget ~a 1) 'z `(aget ~a 2)
               'vx `(nth ~v 0 0.0) 'vy `(nth ~v 1 0.0) 'vz `(nth ~v 2 0.0)}
              body)))))

  (defmacro with-v3-or-vs
    [src v & body]
    (let [[a b x y z vx vy vz] (repeatedly 8 gensym)]
      `(let [~a ~src, ~x (aget ~a 0), ~y (aget ~a 1), ~z (aget ~a 2)]
         (if (instance? ~'Vec3 ~v)
           ...
           (let [~vx (nth ~v 0 0.0), ~vy (nth ~v 1 0.0), ~vz (nth ~v 2 0.0)]
             ~@(postwalk-replace {'x x 'y y 'z z 'vx vx 'vy vy 'vz vz} body))))))
