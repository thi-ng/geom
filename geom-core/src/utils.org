...
#+BEGIN_SRC clojure :noweb-ref tessellate
  ...
  (defn tessellate-with-first
    ...
    (if (> (count points) 3)
      (let [[v0 & more] points]
        (mapv (fn [[a b]] [v0 a b]) (partition 2 1 more)))
      [points]))
...
#+BEGIN_SRC clojure :noweb-ref normals
  (defn ortho-normal
    ([[a b c]] (m/normalize (g/cross (m/- b a) (m/- c a))))
    ([a b] (m/normalize (g/cross a b)))
    ([a b c] (m/normalize (g/cross (m/- b a) (m/- c a)))))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref triangles
  ...
  (defn norm-sign3
    [a b c] (g/mag (g/cross (m/- b a) (m/- c a))))
  ...
  (defn clockwise3?
    [a b c n] (pos? (g/dot (g/cross (m/- b a) (m/- c a)) n)))
