...
#+BEGIN_SRC clojure :noweb-ref vec2
  (deftype Vec2
    ...
    ,#+cljs [x y ^:mutable _hashc ^:mutable _hasheq]
#+END_SRC
*** Clojure protocols
#+BEGIN_SRC clojure :noweb-ref vec2
...
#+BEGIN_SRC clojure :noweb-ref vec2
  ...
  (rotate
   ...
   (let [s (Math/sin theta) c (Math/cos theta)]
     (Vec2. (mm/msub x c y s) (mm/madd x s y c) -1 -1)))
  g/PRotate3D
  (rotate-x
...
#+BEGIN_SRC clojure :noweb-ref vec2
  g/PMathOps
  (abs [_] (Vec2. (m/abs x) (m/abs y) -1 -1))
  (+ [_] _)
  (+ [_ a] (vec2-op-1 + _ a 0.0))
  (+ [_ a b] (vec2-op-2 + _ a b 0.0 0.0))
  (+ [_ a b c] (vec2-op-3 + _ a b c 0.0))
...
#+BEGIN_SRC clojure :noweb-ref vec2
  ...
  (min
   [_ [ax ay]]
   (Vec2. (min x ax) (min y ay) -1 -1))
  (min
   [_ [ax ay] [bx by]]
   (Vec2. (min (min x ax) bx) (min (min y ay) by) -1 -1))
  (max
   [_ [ax ay]]
   (Vec2. (max x ax) (max y ay) -1 -1))
  (max
   [_ [ax ay] [bx by]]
   (Vec2. (max (max x ax) bx) (max (max y ay) by) -1 -1))
#+END_SRC
*** PMagnitude
#+BEGIN_SRC clojure :noweb-ref vec2
  g/PMagnitude
...
#+BEGIN_SRC clojure :noweb-ref vec2
  ...
  (reflect
   [_ [rx ry :as r]]
    (let [d (* (g/dot _ r) 2.0)]
      (Vec2. (mm/msub rx d x) (mm/msub ry d y) -1 -1)))
#+END_SRC
*** PInvert
#+BEGIN_SRC clojure :noweb-ref vec2
  g/PInvert
...
#+BEGIN_SRC clojure :noweb-ref vec2
  ...
  (mix
   [_ [ax ay]]
   (Vec2. (mm/submadd ax x 0.5 x) (mm/submadd ay y 0.5 y) -1 -1))
  (mix
   [_ [ax ay] t]
   (Vec2. (mm/submadd ax x t x) (mm/submadd ay y t y) -1 -1))
  (mix
   ...
   (let [ux (mm/submadd ax x u x)
     ...
     (Vec2.
      ...
      (mm/submadd (mm/submadd cy by u by) uy v uy) -1 -1)))
#+END_SRC
*** PHeading
#+BEGIN_SRC clojure :noweb-ref vec2
  g/PHeading
...
#+BEGIN_SRC clojure :noweb-ref vec2
  g/PPolar
  (as-polar
   [_] (Vec2. (g/mag _) (g/heading-xy _) -1 -1))
  (as-cartesian
   [_] (Vec2. (* x (Math/cos y)) (* x (Math/sin y)) -1 -1))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref vec3
  (deftype Vec3
    #+clj [^double x ^double y ^double z
           ^:unsynchronized-mutable _hashc
           ^:unsynchronized-mutable _hasheq]
    #+cljs [x y z ^:mutable _hashc ^:mutable _hasheq]
#+END_SRC
*** Clojure protocols
#+BEGIN_SRC clojure :noweb-ref vec3
  ,#+clj clojure.lang.ILookup
  ...
  ,#+clj (more
         [_] <<tpl-rest3>>)
  ,#+clj (cons
         [_ w] (persistent! (conj! (transient _) w)))
  ,#+clj (peek
         [_] z)
  ,#+clj (pop
         [_] (Vec2. x y -1 -1))
...
#+BEGIN_SRC clojure :noweb-ref vec3
  ...
  (rotate-x
   [_ theta]
   (let [s (Math/sin theta) c (Math/cos theta)]
     (Vec3. x (mm/msub y c z s) (mm/madd y s z c) -1 -1)))
  (rotate-y
   [_ theta]
   (let [s (Math/sin theta) c (Math/cos theta)]
     (Vec3. (mm/madd x c z s) y (mm/msub z c x s) -1 -1)))
  (rotate-z
   [_ theta]
   (let [s (Math/sin theta) c (Math/cos theta)]
     (Vec3. (mm/msub x c y s) (mm/madd x s y c) z -1 -1)))
  (rotate-around-axis
   [_ [ax ay az] theta]
...
#+BEGIN_SRC clojure :noweb-ref vec3
  g/PMathOps
  (abs [_] (Vec3. (m/abs x) (m/abs y) (m/abs z) -1 -1))
  (+ [_] _)
  (+ [_ a] (vec3-op-1 + _ a 0.0))
  (+ [_ a b] (vec3-op-2 + + _ a b 0.0 0.0))
  (+ [_ a b c] (vec3-op-3 + _ a b c 0.0))
  ...
  (min
   [_ [ax ay az]]
   (Vec3. (min x ax) (min y ay) (min z az) -1 -1))
  (min
   [_ [ax ay az] [bx by bz]]
   (Vec3.
    (min (min x ax) bx)
  ...
  (max
   [_ [ax ay az]]
   (Vec3. (max x ax) (max y ay) (max z az) -1 -1))
  (max
   [_ [ax ay az] [bx by bz]]
   (Vec3.
    (max (max x ax) bx)
...
#+BEGIN_SRC clojure :noweb-ref vec3
  ...
  (reflect
   [_ [rx ry rz :as r]]
    (let [d (* (g/dot _ r) 2.0)]
      (Vec3. (mm/msub rx d x) (mm/msub ry d y) (mm/msub rz d z) -1 -1)))
#+END_SRC
*** PInvert
#+BEGIN_SRC clojure :noweb-ref vec3
  g/PInvert
...
#+BEGIN_SRC clojure :noweb-ref vec3
  ...
  (mix
   ...
   (Vec3.
    ...
    (mm/submadd az z 0.5 z) -1 -1))
  (mix
   [_ [ax ay az] t]
   (Vec3.
    (mm/submadd ax x t x)
    (mm/submadd ay y t y)
    (mm/submadd az z t z) -1 -1))
  (mix
   ...
   (let [ux (mm/submadd ax x u x)
     ...
     (Vec3.
      ...
      (mm/submadd (mm/submadd cz bz u bz) uz v uz) -1 -1)))
#+END_SRC
*** PHeading
#+BEGIN_SRC clojure :noweb-ref vec3
  g/PHeading
...
*** Override print methods
#+BEGIN_SRC clojure :noweb-ref vec3
  #+clj (defmethod print-method Vec3 [^Vec3 o ^java.io.Writer w] (.write w (.toString o)))
#+END_SRC
** Vector operations
*** 2D
  ...
  (defn vec2-op-1
    ...
    (if (instance? Vec2 a)
      (Vec2.
       ...
       (op (.-y _) (.-y ^Vec2 a)) -1 -1)
      (if (number? a)
        (Vec2. (op (.-x _) a) (op (.-y _) a) -1 -1)
        (if (instance? Vec2 (first a))
          (vec2-op-seq op _ a)
          (Vec2.
           (op (.-x _) (nth a 0 d))
           (op (.-y _) (nth a 1 d)) -1 -1)))))
    ...
    (if (instance? Vec2 a)
      (if (instance? Vec2 b)
        (Vec2. ;; v v
         ...
         (op (op (.-y _) (.-y ^Vec2 a)) (.-y ^Vec2 b)) -1 -1)
        (if (number? b)
          (Vec2. ;; v n
           (op (op (.-x _) (.-x ^Vec2 a)) b)
           (op (op (.-y _) (.-y ^Vec2 a)) b) -1 -1)
          (Vec2. ;; v ?
           (op (op (.-x _) (.-x ^Vec2 a)) (nth b 0 d2))
           (op (op (.-y _) (.-y ^Vec2 a)) (nth b 1 d2)) -1 -1)))
        ...
        (if (and an? bn?)
          ...
          (let [[ax ay] (if an? [a a] a)
            ...
            (Vec2. ;; ? ?
             ...
             (op (op (.-y _) (or ay d1)) (or by d2)) -1 -1))))))
    ...
    (if (instance? Vec2 a)
      (if (instance? Vec2 b)
        (Vec2. ;; v v
         ...
         (op2 (op (.-y _) (.-y ^Vec2 a)) (.-y ^Vec2 b)) -1 -1)
        (if (number? b)
          (Vec2. ;; v n
           (op2 (op (.-x _) (.-x ^Vec2 a)) b)
           (op2 (op (.-y _) (.-y ^Vec2 a)) b) -1 -1)
          (Vec2. ;; v ?
           (op2 (op (.-x _) (.-x ^Vec2 a)) (nth b 0 d2))
           (op2 (op (.-y _) (.-y ^Vec2 a)) (nth b 1 d2)) -1 -1)))
          ...
          (if (and an? bn?)
            ...
            (let [[ax ay] (if an? [a a] a)
              ...
              (Vec2. ;; ? ?
               ...
               (op2 (op (.-y _) (or ay d1)) (or by d2)) -1 -1)))))))
    ...
    (loop [ax (.-x acc), ay (.-y acc), az (.-z acc), as as]
      (if as
        ...
        (let [[bx by bz] (if (number? b) [b b b] b)]
          (Vec3. (op2 ax bx) (op2 ay by) (op2 az bz) -1 -1)))))
  ...
  (defn vec3-op-1
    ...
    (if (instance? Vec3 a)
      (Vec3.
       ...
       (op (.-z _) (.-z ^Vec3 a)) -1 -1)
      (if (number? a)
        (Vec3. (op (.-x _) a) (op (.-y _) a) (op (.-z _) a) -1 -1)
        (if (instance? Vec3 (first a))
          (vec3-op-seq op _ a)
          (Vec3.
           (op (.-x _) (nth a 0 d))
           (op (.-y _) (nth a 1 d))
           (op (.-z _) (nth a 2 d)) -1 -1)))))
      ...
      (if (and (sequential? a) (instance? Vec3 (first a)))
        ...
        (let [[ax ay az] (if (number? a) [a a a] a)
          ...
          (Vec3. ;; ? ? ?
           ...
           (op2 (op (.-z _) (or az d1)) (or bz d2)) -1 -1)))))
        ...
        (if (and an? bn? cn?)
          ...
          (let [[ax ay az] (if (number? a) [a a a] a)
            ...
            (Vec3. ;; ? ? ?
             ...
             (op (op (op (.-z _) (or az d)) (or bz d)) (or cz d)) -1 -1))))))
#+END_SRC
** Constants
#+BEGIN_SRC clojure :noweb-ref constants
  (def V2 (Vec2. 0.0 0.0 -1 -1))
  (def V3 (Vec3. 0.0 0.0 0.0 -1 -1))
  
  (def V2X (Vec2. 1.0 0.0 -1 -1))
  (def V2Y (Vec2. 0.0 1.0 -1 -1))
  
  (def V3X (Vec3. 1.0 0.0 0.0 -1 -1))
  (def V3Y (Vec3. 0.0 1.0 0.0 -1 -1))
  (def V3Z (Vec3. 0.0 0.0 1.0 -1 -1))
  
  (def V2INF- (Vec2. INF- INF- -1 -1))
  (def V2INF+ (Vec2. INF+ INF+ -1 -1))
  
  (def V3INF- (Vec3. INF- INF- INF- -1 -1))
  (def V3INF+ (Vec3. INF+ INF+ INF+ -1 -1))
#+END_SRC
** Constructors
...
#+BEGIN_SRC clojure :noweb-ref ctors
  (defn vec2
    ...
    ([x] (cond
          (instance? Vec2 x) x
          (sequential? x) (Vec2. (nth x 0 0.0) (nth x 1 0.0) -1 -1)
          (number? x) (Vec2. x x -1 -1)
          (map? x) (Vec2. (get x :x 0.0) (get x :y 0.0) -1 -1)
  ...
  (defn vec3
    ...
    ([x] (cond
          (instance? Vec3 x) x
          (sequential? x) (Vec3. (nth x 0 0.0) (nth x 1 0.0) (nth x 2 0.0) -1 -1)
          (number? x) (Vec3. x x x -1 -1)
          (map? x) (Vec3. (get x :x 0.0) (get x :y 0.0) (get x :z 0.0) -1 -1)
          :default (err/type-error! "Vec3" x)))
    ([x y] (Vec3. x y 0.0 -1 -1))
    ([x y z] (Vec3. x y z -1 -1)))
        ...
        (let [n (name k) c (count n)]
          (case c
            2 (Vec2.
               ...
               (lookup2 _ (nth n 1) default) -1 -1)
            3 (Vec3.
               (lookup2 _ (nth n 0) default)
               (lookup2 _ (nth n 1) default)
               (lookup2 _ (nth n 2) default) -1 -1)
            (or default (err/key-error! k)))))))
        ...
        (let [n (name k) c (count n)]
          (case c
            2 (Vec2.
               ...
               (lookup3 _ (nth n 1) default) -1 -1)
            3 (Vec3.
               (lookup3 _ (nth n 0) default)
               (lookup3 _ (nth n 1) default)
               (lookup3 _ (nth n 2) default) -1 -1)
            (or default (err/key-error! k)))))))

  (defn swizzle3-assoc
    [^Vec3 _ k v]
    (case k
      :x (Vec3. v (.-y _) (.-z _) -1 -1)
      :y (Vec3. (.-x _) v (.-z _) -1 -1)
      :z (Vec3. (.-x _) (.-y _) v -1 -1)
      0 (Vec3. v (.-y _) (.-z _) -1 -1)
      1 (Vec3. (.-x _) v (.-z _) -1 -1)
      2 (Vec3. (.-x _) (.-y _) v -1 -1)
