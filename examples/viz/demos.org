...
* Contents                                                         :toc_4_gh:
 - [[#geomviz-examples][geom.viz examples]]
     - [[#running-all-examples-from-repl][Running all examples from REPL]]
         - [[#todo-create-fn-to-re-use-for-all-modules][TODO create fn to re-use for all modules]]
...
Once you've tangled this document (see [[../../README.org#building--testing-this-project][README.org]]), the following
examples can be found in the =/babel/examples/viz= folder. Launching a
REPL from within the =babel= directory, all examples can be run with
this command and will create a number of SVG files in the same
directory.

#+BEGIN_SRC clojure
  (load-file "examples/viz/all.clj")
#+END_SRC

*** TODO create fn to re-use for all modules

#+BEGIN_SRC clojure :tangle ../../babel/examples/viz/all.clj :noweb yes :mkdirp yes :padline no
  ;; all.clj
  (doseq [f (rest (file-seq (java.io.File. "examples/viz")))
          :when (not= "all.clj" (.getName f))]
