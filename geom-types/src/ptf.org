  ...
  (-> (scene/base-scene {:width 640 :height 360})
      (scene/scene-add-main-mesh
       (ptf/sweep-mesh
        ...
        :align? true)
       {:bounds (a/aabb 1.5) :target [0 0.5 -2] :rx (- m/THIRD_PI)})
...
#+BEGIN_SRC clojure :tangle ../babel/examples/ptf-knot2.clj :mkdirp yes :padline no
  ...
  (require '[thi.ng.geom.types.utils.ptf :as ptf])
  (require '[thi.ng.common.math.core :as m :refer [TWO_PI]])
  (require '[thi.ng.luxor.scenes :as scene])
  (require '[thi.ng.luxor.io :as lio])
  ...
  (defn weave-strand
    ...
    (-> (mapv
         #(->> (vec2 r (+ (* % delta) theta))
               (g/as-cartesian)
               (ptf/sweep-point (p %) (n %) (b %)))
         (range (count p)))
        (ptf/sweep-mesh profile :align? true)))

  (defn weave-mesh
    [base r strands twists profile]
    (let [delta (/ (* twists TWO_PI) (dec (count (first base))))]
      (reduce
       #(g/into %1 (weave-strand base 0.5 %2 delta profile))
       (bm/basic-mesh)
       (range 0 TWO_PI (/ TWO_PI strands)))))

  (def knot-mesh
    (-> (mapv cinquefoil (m/norm-range 400))
        (ptf/compute-frames)
        (ptf/align-frames)
        (weave-mesh 0.5 10 7 (g/vertices (c/circle 0.1) 20))))
