...
#+BEGIN_SRC clojure :noweb-ref ptf
  ...
  (defn weave-mesh
    ...
    (let [delta (/ (* twists TWO_PI) (dec (count (first base))))]
      (reduce
       #(g/into %1 (weave-strand base r (* %2 TWO_PI) delta profile))
...
These two brief example demonstrate usage of the =sweep-mesh= function
in combination with [[http://thi.ng/luxor][thi.ng/luxor's]] test scene setup. The first example
computes a [[https://en.wikipedia.org/wiki/Torus_knot][Cinquefoil knot]] (a (5,2)-torus knot) and sweeps a circle
along the path using PTF. The second demo also uses the same knot as
basis, but instead of sweeping it directly, it first computes 10
intertwining strands weaving around the path and each strand is swept
as mesh individually. Both examples are tangled into the
=/babel/examples= directory, but will only work in a REPL which has
both the =geom= and =luxor= libs on its classpath.
...
#+BEGIN_SRC clojure :tangle ../babel/examples/ptf-knot.clj :mkdirp yes :padline no
  ...
  (require '[thi.ng.common.math.core :as m :refer [THIRD_PI TWO_PI]])
  ...
  (-> (scene/base-scene {:width 640 :height 360})
      (scene/scene-add-main-mesh
       (ptf/sweep-mesh
        ...
        :align? true)
       {:bounds (a/aabb 1.5) :target [0 0.5 -2] :rx (- THIRD_PI)})
...
#+BEGIN_SRC clojure :tangle ../babel/examples/ptf-knot2.clj :mkdirp yes :padline no
  (require '[thi.ng.geom.core :as g])
  (require '[thi.ng.geom.core.vector :refer [vec2 vec3]])
  (require '[thi.ng.geom.aabb :as a])
  (require '[thi.ng.geom.circle :as c])
  (require '[thi.ng.geom.types.utils.ptf :as ptf])
  (require '[thi.ng.common.math.core :as m :refer [THIRD_PI TWO_PI]])
  ...
  (def knot-mesh
    (-> (mapv cinquefoil (m/norm-range 400))
        (ptf/compute-frames)
        (ptf/align-frames)
        (ptf/weave-mesh 0.5 10 7 (g/vertices (c/circle 0.1) 20))
        ;;(ptf/weave-mesh 0.8 6 12 (g/vertices (c/circle 0.1) 20))
        ))
