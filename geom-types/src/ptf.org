...
These two brief examples demonstrate usage of the =sweep-mesh= and
=weave-mesh= functions in combination with [[http://thi.ng/luxor][thi.ng/luxor's]] test scene
...
first computes N strands, each rotating/weaving around the path at
...
Two example renders to illustrate the =weave-mesh= approach/function:
...
#+BEGIN_SRC clojure :tangle ../babel/examples/ptf-knot2.clj :mkdirp yes :padline no
  ...
  (-> (scene/base-scene {:width 640 :height 360})
      (scene/scene-add-main-mesh
       (ptf/weave-mesh knot 0.5 10 7 (g/vertices (c/circle 0.1) 20))
        ;;(ptf/weave-mesh knot 0.8 6 12 (g/vertices (c/circle 0.1) 20))
       {:bounds (a/aabb 1.5) :target [0 0.5 -2] :rx (- THIRD_PI)})
