...
#+BEGIN_SRC clojure :noweb-ref ptf
  ...
  (defn sweep-profile
    [profile attribs [points _ norms bnorms]]
                               ...
                               (map-indexed
                                (fn [j [a b]]
                                  (attr/generate-face-attribs
                                   ...
                                   (+ fid j)
                                   attribs
                                   (assoc opts :u (double (/ j nprof))))))
                               (concat faces))]
                [faces curr (inc i) (+ fid nprof1)]))
            [nil (conj frame0 (first frame0)) 0 0])
           (first))))

  (defn sweep-mesh
    [points profile & [{:keys [mesh align? attribs]}]]
    (let [frames (compute-frames points)
          frames (if align? (align-frames frames) frames)]
      (->> frames
           (sweep-profile profile attribs)
