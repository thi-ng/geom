...
#+BEGIN_SRC clojure :noweb-ref helpers  
  (defn tangent-points
    [{p :p :as c} q]
    (let [m (g/mix p q)]
      (gu/intersect-circle-circle? c (circle m (g/dist m p)))))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref impl
  ...
  (intersect-shape
   [_ s]
   (cond
    (instance? thi.ng.geom.types.Circle2 s) (gu/intersect-circle-circle? _ s)
    (instance? thi.ng.geom.types.Rect2 s) (gu/intersect-aabb-sphere? s _)
