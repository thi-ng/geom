...
#+BEGIN_SRC clojure :noweb-ref ctors
  (defn rect
    ([] (thi.ng.geom.types.Rect2. (vec2) 1.0 1.0))
    ([w] (thi.ng.geom.types.Rect2. (vec2) w w))
    ([p q]
       (if (coll? p)
         (if (coll? q)
           ...
           (thi.ng.geom.types.Rect2. (vec2 p) q q))
         (thi.ng.geom.types.Rect2. (vec2) p q)))
    ([x y w]
       (if (number? x)
         (thi.ng.geom.types.Rect2. (vec2 x y) w w)
         (thi.ng.geom.types.Rect2. (vec2 x) y w)))
    ([x y w h] (thi.ng.geom.types.Rect2. (vec2 x y) w h)))
#+END_SRC
