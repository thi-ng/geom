...
freeform box, which can be freely rotated & transformed. An AABB can
be considered a logical subtype of this type and an AABB can be
converted into a Box via the =box= constructor function defined in
this namespace.
...
#+BEGIN_SRC clojure :noweb-ref ctors
  (defn box
    ...
    ([x]
       (cond
        (number? x)     (box (vec3) x)
        (map? x)        (box (:p x) (:size x))
        (sequential? x) (if (== 8 (count x))
                          (thi.ng.geom.types.Box. (mapv vec3 x))
                          (err/illegal-arg!
