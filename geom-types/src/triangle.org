  ...
  (defn equilateral2
    ...
    ([a b]
       (let [a (vec2 a) b (vec2 b)
             dir (m/- a b)
             n (g/normal dir)
             c (-> n (m/normalize (mm/mul (g/mag dir) SQRT3 0.5)) (m/+ (m/mix a b)))]
         (triangle2 a b c)))
    ([x1 y1 x2 y2]
  ...
  (as-mesh
   ...
   ([_ opts]
    (g/add-face
     ...
     (attr/generate-face-attribs (mapv vec3 (:points _)) 0 (:attribs opts) opts))))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref impl3
  ...
  (classify-point
   [_ p]
   (let [a (first (:points _))
         n (gu/ortho-normal (:points _))
         w (- (g/dot n a))]
     (-> n (g/dot p) (+ w) (m/signum *eps*))))
#+END_SRC
**** TODO IExtrude
#+BEGIN_SRC clojure :noweb-ref impl3
  g/IExtrude
