  ...
  (defn- add-face*
    ...
    (let [[vindex vids] (idx/index-coll (:vertices mesh) verts)
      ...
      (thi.ng.geom.types.IndexedMesh.
       vindex
       (conj (:faces mesh) (thi.ng.geom.meshface.IndexedMeshFace. vids aids nil))
       aindex)))
...
#+BEGIN_SRC clojure :noweb-ref impl
  g/ITessellate
  (tessellate
   ([_]      (g/tessellate _ {:attribs (zipmap (keys (:attribs _)) (repeat m/mix))}))
...
#+BEGIN_SRC clojure :noweb-ref impl
  ...
  (scale
   ([_ s]
    (tu/transform-mesh _ add-face* #(m/* % s)))
   ([_ sx sy sz]
    (tu/transform-mesh _ add-face* #(m/* % sx sy sz))))
  (scale-size
   [_ s]
   (let [c (g/centroid _)]
     (tu/transform-mesh _ add-face* #(m/madd (m/- % c) s c))))
  g/ITranslate
  (translate
   [_ t] (tu/transform-mesh _ add-face* #(m/+ % t)))
  g/ITransform
  (transform
   [_ tx]
