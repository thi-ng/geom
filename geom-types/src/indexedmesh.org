    ...
    (let [id (get (:index index) item)]
      (if id
        {:index (assoc (dissoc (:index index) item) newitem id)
         :items (into
                 (conj (subvec (:items index) 0 id) newitem)
                 (subvec (:items index) (inc id))) ;; FIXME ID != vector index after 1st reindex, use map?
         :next  (:next index)}
        index)))
...
#+BEGIN_SRC clojure :noweb-ref impl
  ...
  (face-normals
   ...
   (if-let [fns (-> _ :attribs :fnorm)]
     (:items fns)
     (if force? (-> (g/compute-face-normals _) :attribs :fnorm :items))))
  (face-normal
   [_ f]
   ((-> _ :attribs :fnorm :items)
  ...
  (compute-face-normals
   ...
   (let [vertices (:vertices _)]
     (loop [fnorms (make-index), faces' (transient []), faces (:faces _)]
       (if faces
         (let [^thi.ng.geom.meshface.IndexedMeshFace f (first faces)
               fverts     (attrib-values vertices (.-vertices f))
               [fnorms n] (index-item fnorms (gu/ortho-normal fverts))
