...
#+BEGIN_SRC clojure :noweb-ref helpers
  ...
  (defn vertices-planar?
    [[a b c :as verts]]
    (or (< (count verts) 4)
        (let [n (g/ortho-normal* a b c)]
          (every? #(m/delta= n (g/ortho-normal* %))
...
#+BEGIN_SRC clojure :noweb-ref impl
  ...
  (center
   ([_] (g/center _ (g/vec3)))
   ([_ o] (g/transform _ (g/translate g/M44 (g/- o (g/centroid _))))))
  (centroid
   [_] (g/centroid* (keys (:vertices _))))
#+END_SRC
   ...
   (loop [norms (transient #{}), fnorms (transient {}), faces (:faces _)]
     (if faces
       (let [[a b c :as f] (first faces)
             [norms n] (d/index! norms (g/ortho-normal* a b c))]
