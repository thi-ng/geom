...
#+BEGIN_SRC emacs-lisp :noweb yes :var type="" :results output
  ...
  (center
   ...
   ([{[p q] :points} o]
    (let [c (m/- o (m/mix p q))]
      ({{type}}. [(m/+ p c) (m/+ q c)]))))
  (centroid [{p :points}] (m/mix (p 0) (p 1)))"))
#+END_SRC
*** IBounds
#+NAME: tpl-bounds
...
#+BEGIN_SRC emacs-lisp :noweb yes :var type="" :results output
  ...
  (scale
   ([{p :points} s]
    ({{type}}. [(m/* (p 0) s) (m/* (p 1) s)]))
   ([{p :points} sx sy]
    ({{type}}. [(m/* (p 0) sx sy) (m/* (p 1) sx sy)])))
  (scale-size
   [{[p q] :points} s]
   (let [c (m/mix p q)]
...
#+BEGIN_SRC clojure :noweb-ref impl2
  ...
  (classify-point
   [{[p q] :points} v]
   (m/signum (g/dot (m/- (vec2 v) p) (g/normal (m/- q p))) *eps*))
#+END_SRC
**** IExtrude                                                           :template:
#+BEGIN_SRC clojure :noweb-ref impl2 :noweb yes
  <<tpl-extrude>>
...
#+BEGIN_SRC clojure :noweb-ref impl3
  ...
  (bounding-sphere
   [{[p q] :points}]
   (tu/bounding-sphere (m/mix p q) (* 0.5 (g/dist p q))))
#+END_SRC
**** ICenter                                                       :template:
#+BEGIN_SRC clojure :noweb-ref impl3 :noweb yes
