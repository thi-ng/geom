...
#+BEGIN_SRC clojure :noweb-ref impl
  ...
  (bounding-sphere
   [{p :p size :size}]
   (let [ext (m/* size 0.5)]
     (tu/bounding-sphere (m/+ p ext) (g/mag ext))))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref impl
  ...
  (union
   ...
   (let [p (m/min pa pb)]
     (thi.ng.geom.types.AABB. p (m/- (m/max (m/+ pa sa) (m/+ pb sb)) p))))
  (intersection
   [_ b]
   (let [pa (:p _) qa (m/+ pa (:size _))
         pb (:p b) qb (m/+ pb (:size b))
         p' (m/max pa pb)
         q' (m/min qa qb)
         s' (m/- q' p')]
     (if (every? #(>= % 0) s')
...
#+BEGIN_SRC clojure :noweb-ref impl
  ...
  g/IScale
  (scale
   ([_ s] (thi.ng.geom.types.AABB. (m/* (:p _) s) (m/* (:size _) s)))
   ([_ sx sy] (thi.ng.geom.types.AABB. (m/* (:p _) sx sy) (m/* (:size _) sx sy)))
   ([_ sx sy sz] (thi.ng.geom.types.AABB. (m/* (:p _) sx sy sz) (m/* (:size _) sx sy sz))))
  (scale-size
   [_ s]
   (let [s' (m/* (:size _) s)]
     (thi.ng.geom.types.AABB. (m/madd (m/- s' (:size _)) -0.5 (:p _)) s')))
  
  g/ITranslate
  (translate
   [_ t] (thi.ng.geom.types.AABB. (m/+ (:p _) t) (:size _)))
