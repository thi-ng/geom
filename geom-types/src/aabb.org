...
#+BEGIN_SRC clojure :noweb-ref ctors
  ...
  (defn union-aabb
    "Takes 2 AABB's and returns the AABB of their union, enclosing both."
  ...
  (defn coll-bounds
    [[x & more]]
    (reduce union-aabb (g/bounds x) (map g/bounds more)))
#+END_SRC

** Type specific functions
#+BEGIN_SRC clojure :noweb-ref helpers
  (defn map-point
    "Takes an AABB and point, returns point in normalized UVW coords in
    local box space, where [0,0,0] is equivalent to the AABB's min point
    and [1,1,1] the point diagonally opposite. If any of the resulting
    coordinates is outside the 0.0 .. 1.0 interval, the orginal point
    was outside the box."
    [box p]
    (g/div (g/- p (:p box)) (:size box)))
  
  (defn unmap-point
    "Inverse operation of map-point. Maps point in local UWV coordinates
    to its world position."
    [box p]
    (g/madd p (:size box) (:p box)))
  
  (defn fit-all-into-aabb
    "Takes an AABB and seq of entities, proportionally scales and
    repositions all items to fit into given box. Returns lazyseq of
    transformed entities."
    [box coll]
    (let [b (coll-bounds coll)
          s (reduce min (g/div (:size box) (:size b)))
          b' (g/center (g/scale b s) (g/centroid box))]
      (map
       #(-> %
           (g/center (unmap-point b' (map-point b (g/centroid %))))
           (g/scale-size s))
       coll)))
#+END_SRC
...
#+BEGIN_SRC clojure :tangle ../babel/test/cljx/thi/ng/geom/types/test/aabb.cljx :noweb yes :mkdirp yes :padline no
  ...
  (deftest test-helpers
    (is (= (a/aabb [-1 -2 -3] [3 5 7]) (a/union-aabb (a/aabb [-1 -2 -3] 1) (a/aabb [1 2 3] 1))) "union")
    (is (= (a/aabb) (a/union-aabb (a/aabb) (a/aabb))) "union self")
    (is (= (a/aabb) (a/coll-bounds (a/fit-all-into-aabb (a/aabb) [(s/sphere -1 1) (a/aabb 2 1)])))
      "fit-all-into-aabb1")
    (is (= (a/aabb [0.5 1.5 0] [1 1 5])
           (a/coll-bounds (a/fit-all-into-aabb (a/aabb 2 4 5) [(s/sphere 1 1) (a/aabb [0 1 9] 1)])))
      "fit-all-into-aabb2"))
#+END_SRC
