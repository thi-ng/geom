...
* Contents                                                         :toc_4_gh:
 - [[#webgl-examples--test-scenes][WebGL examples & test scenes]]
     ...
     - [[#todo-example-6-morphogen--shadow-mapping][TODO Example 6: Morphogen & shadow mapping]]
...
Live version: http://demo.thi.ng/geom/webgl/ptf-knot/
...
#+BEGIN_SRC clojure :tangle ../babel/test/thi/ng/geom/webgl/example05.cljs :noweb yes :mkdirp yes :padline no
  ...
  (defn ^:export demo
    ...
    (let [gl        (gl/gl-context "main")
          ...
          tex       (buf/load-texture gl {:src "dev/cubev.png" :flip true
         ...
         (when @tex-ready
           ...
           (buf/draw-arrays-with-shader
            gl (update model :uniforms merge
                       {:time t
                        :model (-> M44 (g/rotate-x (* t 0.73)) (g/rotate-y (* t 2)))})))
         true))))
#+END_SRC

Live version: http://demo.thi.ng/geom/webgl/ptf-knot2/ & http://demo.thi.ng/geom/webgl/ptf-knot3/
  ...
  (def shader-spec
    {:vs (->> "void main() {
     ...
     :fs (->> "void main() {
                ...
                float fog = fogLinear(length(vPos), 1.0, 8.0);
                col = mix(col, Kf, fog);
...
** TODO Example 6: Morphogen & shadow mapping
