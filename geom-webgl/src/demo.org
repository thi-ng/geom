...
#+BEGIN_SRC js
  thi.ng.geom.webgl.example01.demo()
#+END_SRC
          ...
          model     (-> (a/aabb 1)
                        ...
                        (merge
                         {:uniforms {:proj (gl/perspective 45 view-rect 0.1 100.0)
                          ...
                          :shader   (sh/make-shader-from-spec gl (basic/make-shader-spec-3d true))})
                        (buf/make-attribute-buffers-in-spec gl gl/static-draw))]
      (anim/animate
       (fn [[t frame]]
         (gl/set-viewport gl view-rect)
...
#+BEGIN_SRC clojure :tangle ../babel/test/thi/ng/geom/webgl/example02a.cljs :noweb yes :mkdirp yes :padline no
  (ns thi.ng.geom.webgl.example02a
    <<require-macros>>
    (:require
     <<default-requires>>
    ...
    (let [gl        (gl/gl-context "main")
          ...
          model     (-> (a/aabb 1)
                        ...
                        (merge
                         {:uniforms {:proj (gl/perspective 45 view-rect 0.1 100.0)
                                     :view (mat/look-at (v/vec3 0 0 2) (v/vec3) v/V3Y)}
                          :shader   (sh/make-shader-from-spec gl lambert/shader-spec-attrib)})
                        (buf/make-attribute-buffers-in-spec gl gl/static-draw))]
      (anim/animate
       (fn [[t frame]]
         (gl/set-viewport gl view-rect)
...
#+BEGIN_SRC clojure :tangle ../babel/test/thi/ng/geom/webgl/example03.cljs :noweb yes :mkdirp yes :padline no
  ...
  (defn ^:export demo
    ...
    (let [gl         (gl/gl-context "main")
          ...
          teeth      20
          model      (-> (poly/cog 0.5 teeth [0.9 1 1 0.9])
                         (g/extrude-shell {:mesh    (glm/gl-mesh 1000 #{:fnorm})
                                           :depth   0.1
                                           :inset   0.025
                                           :wall    0.015
                                           :bottom? true})
...
#+BEGIN_SRC clojure :tangle ../babel/test/thi/ng/geom/webgl/example04.cljs :noweb yes :mkdirp yes :padline no
  ...
  (defn ^:export demo
    ...
    (let [gl         (gl/gl-context "main")
          ...
          box        (-> (a/aabb 1)
                         (g/center)
                         (g/as-mesh {:mesh (glm/gl-mesh 12 #{:fnorm})})
                         (gl/as-webgl-buffer-spec {})
                         (buf/make-attribute-buffers-in-spec gl gl/static-draw)
          ...
          planes     (-> (g/as-mesh back {:mesh (glm/gl-mesh 4 #{:fnorm}) :size size})
         ...
         (let [eye  (g/rotate-y eye (Math/sin t))
               ...
               isec (if (:update-ray @state)
                      (let [p (-> (vec3 (:mpos @state) 0)
                                  (mat/unproject-point (m/invert (m/* (:proj uniforms) view)) view-rect)
  ...
  (defn ^:export demo
    ...
    (let [gl        (gl/gl-context "main")
          ...
          model     (-> (a/aabb 1)
                        (g/center)
                        (g/as-mesh
                         {:mesh    (glm/gl-mesh 12 #{:uv})
                          :attribs {:uv (attr/face-attribs (attr/uv-cube-map-v 256 false))}})
                        (gl/as-webgl-buffer-spec {})
                        (merge
                         {:uniforms {:proj (gl/perspective 90 view-rect 0.1 100.0)
                                     :view (mat/look-at (v/vec3 0 0 0.5) (v/vec3) v/V3Y)}
                          :shader   (sh/make-shader-from-spec gl shader-spec)})
                        (buf/make-attribute-buffers-in-spec gl gl/static-draw))
          tex-ready (volatile! true)
         ...
         (when @tex-ready
           ...
           (buf/draw-arrays-with-shader
            gl (update model :uniforms merge
                       {:model (-> M44 (g/rotate-x PI) (g/rotate-y (* t 2)))})))
         true))))
#+END_SRC

Live version: http://demo.thi.ng/geom/webgl/ptf-knot/ & http://demo.thi.ng/geom/webgl/ptf-knot3/

#+BEGIN_SRC clojure :tangle ../babel/test/thi/ng/geom/webgl/example05a.cljs :noweb yes :mkdirp yes :padline no
  (ns thi.ng.geom.webgl.example05a
    <<require-macros>>
    (:require
     <<default-requires>>
     [thi.ng.geom.circle :as c]
  ...
  (def shader-spec
    {:vs (->> "void main() {
     ...
     :fs (->> "void main() {
              ...
              (glsl/glsl-spec [fog/linear fog/exp light/beckmann-specular])
              (glsl/assemble))
     :uniforms {:model     :mat4
                :view      :mat4
                :proj      :mat4
  ...
  (defn knot-nested
    ...
    (-> (mapv cinquefoil (m/norm-range 400))
        ...
        (ptf/sweep-strand-mesh
         #(+ 0.5 (* 0.5 (Math/sin (* 8 TWO_PI (/ % 399.0)))))
  ...
  (defn ^:export demo
    ...
    (let [gl        (gl/gl-context "main")
          ...
          model     (-> #_(knot-simple)
                        ...
                        (gl/as-webgl-buffer-spec {})
                        (merge
                         {:uniforms {:proj (gl/perspective 90 view-rect 0.1 10.0)
                                     :view (mat/look-at (v/vec3 0 0 5) (v/vec3) v/V3Y)}
                          :shader   (sh/make-shader-from-spec gl shader-spec)})
                        (buf/make-attribute-buffers-in-spec gl gl/static-draw)
                        (time))
          tex       (gradient-texture gl 4 1024 {:wrap [gl/clamp-to-edge gl/repeat]})]
      (anim/animate
       (fn [[t frame]]
         (gl/set-viewport gl view-rect)
         (gl/clear-color-buffer gl 0.0 0.0 0.1 1)
         (gl/enable gl gl/depth-test)
         (gl/bind tex 0)
         (buf/draw-arrays-with-shader
          gl (-> model
                 ...
                 (sh/inject-normal-matrix :model :view :normalMat)))
         true))))
#+END_SRC
...
#+BEGIN_SRC clojure :tangle ../babel/test/thi/ng/geom/webgl/example06.cljs :noweb yes :mkdirp yes :padline no
  (ns thi.ng.geom.webgl.example06
    <<require-macros>>
    (:require
     <<default-requires>>
  ...
  (defn cam-set-view
    [cam opts]
    (let [cam (merge cam (select-keys opts [:eye :target :up]))]
      (assoc cam :view (mat/look-at (:eye cam) (:target cam) (:up cam)))))

  (defn cam-set-projection
    [cam opts]
    (let [cam (merge cam (select-keys opts [:fov :aspect :near :far]))]
      (assoc cam :proj (gl/perspective (:fov cam) (:aspect cam) (:near cam) (:far cam)))))

  (defn cam-set-uniforms
    [spec cam]
  ...
  (defn ^:export demo
    ...
    (let [gl             (gl/gl-context "main")
          ...
          fbo            (-> (buf/make-fbo gl)
                             ...
                             (gl/unbind))
          fbo-cam        (perspective-camera {:eye (vec3 0 0 3) :aspect 1.0})
          main-cam       (perspective-camera {:eye (vec3 0 0 2) :fov 90 :aspect view-rect})
      ...
      (anim/animate
       (fn [[t frame]]
         ...
         (doto gl
           ...
           (gl/clear-color-buffer 0.8 0.8 0.8 1)
           (gl/clear-depth-buffer 1)
           (gl/enable gl/depth-test)
           (buf/draw-arrays-with-shader
         ...
         ;; render pass #3: Draw FBO texture as 2D overlay
...
#+BEGIN_SRC clojure
 ;;:tangle ../babel/test/thi/ng/geom/webgl/example06.cljs :noweb yes :mkdirp yes :padline no
  (ns thi.ng.geom.webgl.example06
    <<require-macros>>
    (:require
     <<default-requires>>
