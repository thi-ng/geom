...
#+BEGIN_SRC clojure :noweb-ref helpers
  ...
  (defn fill-vertex-buffer
    ...
    (loop [i 0, coll (seq coll)]
      (if coll
        (recur (into-buffer (first coll) buf i stride) (next coll))
        buf)))
  ...
  (defn face-normals-buffer
    ...
    (let [buf  (arrays/float32 (* (count faces) 3 3))
          nbuf (arrays/float32 3)]
      (loop [idx 0, faces faces]
        (if faces
          (let [n (gu/ortho-normal (first faces))]
            (aset nbuf 0 (.-x ^thi.ng.geom.core.vector.Vec3 n))
            (aset nbuf 1 (.-y ^thi.ng.geom.core.vector.Vec3 n))
            (aset nbuf 2 (.-z ^thi.ng.geom.core.vector.Vec3 n))
            (.set buf nbuf idx)
            (.set buf nbuf (+ idx 3))
            (.set buf nbuf (+ idx 6))
    ...
    (into-buffer [_ buf idx stride]
      (let [t     (type (first _))
            into* (case t
                   ...
                   into-buffer)]
        (case (count _)
          2 (let [idx (into* (first _) buf idx stride)]
              (into* (nth _ 1) buf idx stride))
          3 (let [idx (into* (first _) buf idx stride)
                  idx (into* (nth _ 1) buf idx stride)]
              (into* (nth _ 2) buf idx stride))
          (loop [idx idx, xs _]
            (if xs
