    ...
    (let [vnorms (g/vertex-normals m)
      ...
      (loop [i 0, faces faces]
        (if faces
          (let [f (first faces)]
            (recur (->> i
                        (into-buffer-vec3 (vnorms (f 0)) buf)
                        (into-buffer-vec3 (vnorms (f 1)) buf)
                        (into-buffer-vec3 (vnorms (f 2)) buf))
                   (next faces)))
          buf))))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref impl
  (defn into-buffer-vec3
    [^thi.ng.geom.core.vector.Vec3 v buf idx]
    (aset buf idx       (.-x v))
    (aset buf (+ idx 1) (.-y v))
    (aset buf (+ idx 2) (.-z v))
    (+ idx 3))
...
#+BEGIN_SRC clojure :noweb-ref impl
  (extend-type cljs.core.PersistentVector
    PWebGLConvert
    (into-buffer [_ buf idx]
      (let [into* (if (instance? thi.ng.geom.core.vector.Vec3 (first _))
                   into-buffer-vec3 into-buffer)]
        (loop [idx idx, xs _]
          (if xs
            (recur (into* (first xs) buf idx) (next xs))
            idx)))))
#+END_SRC
