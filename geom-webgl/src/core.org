...
#+BEGIN_SRC clojure :noweb-ref impl
  (extend-type thi.ng.geom.types.Polygon2
    ...
    (as-webgl-buffer-spec
      [_ {:keys [normals stride] :or {stride 3, normals true}}]
      (let [faces (g/tessellate _)
            num-faces (count faces)
            num-verts (* num-faces 3)
            verts (arrays/float32 (* num-verts stride))
            norms (if normals
                    (fill-buffer-vec3 (arrays/float32 (* num-verts 3)) 0 V3Z num-verts 3))]
        (loop [i 0, faces faces]
          (if faces
            (recur (into-buffer (first faces) verts i stride) (next faces))))
        {:vertices     verts
         :normals      norms
         :v-size       stride
         :n-size       3
...
#+BEGIN_SRC clojure :noweb-ref impl
  (extend-type thi.ng.geom.types.GMesh
    PWebGLConvert
    (as-webgl-buffer-spec
      [_ {:keys [vnormals fnormals tessellate stride] :or {stride 3, fnormals true}}]
