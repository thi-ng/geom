...
#+BEGIN_SRC clojure :noweb-ref impl
  (extend-type thi.ng.geom.types.LineStrip2
    ...
    (as-webgl-buffer-spec
      [{:keys [points]} {:keys [stride uv color] :or {stride 2}}]
      (let [numv     (count points)
            c-stride (if color (count color))]
        (cond->
         {:attribs
         ...
         color (assoc-in
                [:attribs :color]
                {:data (fill-buffer
                        (arrays/float32 (* numv (count color)))
                        0 color numv (count color))
                 :size c-stride})))))
#+END_SRC
      ...
      (let [m         (if tessellate (g/tessellate _) _)
        ...
        (cond->
         {:attribs
          ...
          :num-vertices num-verts
          :num-faces    num-faces}
         fnormals (assoc-in [:attribs :normal] {:data (face-normals m) :size 3})
         color    (assoc-in
                   [:attribs :color]
                   {:data (fill-buffer
                           (arrays/float32 (* num-verts c-stride))
      ...
      (let [m         (if tessellate (g/tessellate _) _)
        ...
        (cond->
         {:attribs
          ...
          :num-vertices num-verts
          :num-faces    num-faces}
         normals (assoc-in [:attribs :normal] {:data normals :size 3})
         color   (assoc-in
                  [:attribs :color]
                  {:data (fill-buffer
                          (arrays/float32 (* num-verts c-stride))
