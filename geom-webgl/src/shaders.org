...
#+BEGIN_SRC clojure :tangle ../babel/src/thi/ng/geom/webgl/shaders/phong.cljs :noweb yes :mkdirp yes :padline no
  (ns thi.ng.geom.webgl.shaders.phong
    ...
    (:require
     ...
     [thi.ng.glsl.lighting.phong]))

  (def shader-spec
    {:vs (glsl/assemble
          {:deps [thi.ng.glsl.vertex.normal/spec]
           :src (minified "
    <<phong-vs>>")})
     :fs (glsl/assemble
          {:deps [thi.ng.glsl.lighting.phong/spec]
           :src (minified "
    <<phong-fs>>")})
     :uniforms {:view          :mat4
                :proj          :mat4
...
#+BEGIN_SRC glsl :noweb-ref spot-fs
  ...
  void main(){
    ...
    vec3 excident = (skyLight(worldNormal) +
...
#+BEGIN_SRC glsl :noweb-ref shadow-fs-pass2 :noweb yes
  ...
  void main() {
    ...
    vec3 excident = (0.4 + skyLight(worldNormal) +
                     lambert(lightSurfaceNormal, -lightPosNormal) *
                     influence(lightPosNormal, coneAngle) *
                     attenuation(lightPos) *
                     illuminated);
