...
#+BEGIN_SRC clojure :noweb-ref uniforms
  (def uniform-types
    {:float       ["1fv" ta/float32]
     ...
     :bvec3       ["3iv" ta/int32]
     :bvec4       ["4iv" ta/int32]
     :mat2        ["Matrix2fv" ta/float32]
     :mat3        ["Matrix3fv" ta/float32]
     :mat4        ["Matrix4fv" ta/float32]
                 ...
                 :setter (cond
                           ...
                           (fn [x]
                             (if (number? x)
                               ...
                               (.call setter gl loc (if (ta/typed-array? x) x (u-cast x)))))
                           :else
                           (fn [x]
                             (.call setter gl loc
                                    (if (ta/typed-array? x)
...
#+BEGIN_SRC clojure :tangle ../babel/src/thi/ng/geom/webgl/shaders/overlay.cljs :noweb yes :mkdirp yes :padline no
  ...
  (defn overlay-spec-from-rect
    ...
    ([^WebGLRenderingContext gl r init-shader?]
       (let [vbuf (ta/float32 8)
             [a b c d] (g/vertices r)
             _ (streams/into-buffer [a b d c] vbuf 2 0)
             spec {:attribs  (gl/make-attribute-buffers
                              gl glc/static-draw
