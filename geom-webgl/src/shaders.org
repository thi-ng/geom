...
- Setters for =:vec3= uniforms expect a 3-element vector or an
  ...
  converted into RGB vector (=[r g b]=)
- Values for Matrix uniforms can be specified as vector =[mat transpose?]=
...
#+BEGIN_SRC clojure :noweb-ref uniforms
  ...
  (defn set-uniform
    ...
    (if-let [u-spec (get-in shader [:uniforms id])]
      ((:setter u-spec) val)
      (prn "WARNING: Unknown shader uniform: " id)))
#+END_SRC
...
#+BEGIN_SRC clojure :tangle ../babel/src/thi/ng/geom/webgl/shaders/lambert.cljs :noweb yes :mkdirp yes :padline no
  ...
  (defn draw
    ...
    (buf/draw-arrays gl (sh/inject-normal-matrix spec :model :normalMat)))
#+END_SRC
